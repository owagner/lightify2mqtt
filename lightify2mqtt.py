...
# - requests 2.10.0 -- do manually upgrade using pip if your system has an old one!
#
...
parser.add_argument('--pollfreq', type=int, default=42, help='Polling interval in seconds. Defaults to 42')
...
def handleSetLights(name,msg,datapoint):
...
def handleSetGroups(name,msg,datapoint):
    global groupsByName
    parms={}
    # Find group
    ...
    try:
        ...
        # Seems to be complex type, try JSON
        parms=json.loads(msg.payload.decode("utf-8"))
    parms["idx"]=group["groupId"]
    doRequest("group/set",parms)

def handleSet(typeid,name,msg,datapoint):
    if typeid=="lights":
        handleSetLights(name,msg,datapoint)
    elif typeid=="groups":
        handleSetGroups(name,msg,datapoint)

def messagehandler(mqc,userdata,msg):
    global wakeup
    try:
        args=msg.topic[len(topic):].split("/",4)
        if len(args)==4:
            (cmd,typeid,name,datapoint)=args
        else:
            (cmd,typeid,name)=args
            datapoint=None
        if cmd=="set":
            handleSet(typeid,name,msg,datapoint)
